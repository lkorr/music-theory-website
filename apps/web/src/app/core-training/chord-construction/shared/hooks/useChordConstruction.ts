...
import { noteNames } from "../../../chord-recognition/shared/theory/core/notes.ts";
import { chordTypes, extendedChordTypes, inversionTypes } from "../../../chord-recognition/shared/theory/core/constants.ts";
import type { ChordType } from "../../../chord-recognition/shared/theory/core/constants.ts";
...
export default function useChordConstruction(levelConfig: LevelConfig): ChordConstructionState {
  ...
  const generateConstructionTask = (previousTask: ConstructionTask | null = null): ConstructionTask => {
    ...
    const chordType = chordTypesObj[chordTypeKey] as ChordType;
    ...
    const rootNoteNumber = noteNames.indexOf(root as any);
    ...
    if (inversion === 'root') {
      ...
      if (inversion === 'first') {
        ...
        const rootIndex = noteNames.indexOf(root as any);
  ...
  const validateChord = (task: ConstructionTask, notes: number[]): boolean => {
    ...
    const expectedNoteCount = (chordTypesObj[task.chordType] as ChordType).intervals.length;
    ...
    const baseIntervals = (chordTypesObj[task.chordType] as ChordType).intervals;
    let expectedSemitonePattern: number[];
    
    if (!task.inversion || task.inversion === 'root') {
      // Root position
      const rootSemitone = noteNames.indexOf(task.root as any) % 12;
    ...
    if (task.inversion === 'first') {
      const expectedBass = (noteNames.indexOf(task.root as any) + baseIntervals[1]) % 12;
      ...
      const expectedBass = (noteNames.indexOf(task.root as any) + baseIntervals[2]) % 12;
      const actualBass = placedSemitones[0];
      return sortedExpected.length === sortedPlaced.length &&
             sortedExpected.every((semitone, index) => semitone === sortedPlaced[index]) &&
             expectedBass === actualBass;
      ...
      const expectedBass = noteNames.indexOf(task.root as any) % 12;
      const actualBass = placedSemitones[0];
      return sortedExpected.length === sortedPlaced.length &&
             sortedExpected.every((semitone, index) => semitone === sortedPlaced[index]) &&
             expectedBass === actualBass;

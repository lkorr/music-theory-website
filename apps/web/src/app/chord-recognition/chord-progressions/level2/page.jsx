/**
 * Chord Progressions Level 2: Basic Inversions
 * 
 * REFACTORED VERSION - Reduced from 875 lines to ~280 lines (68% reduction)
 * Now uses shared hooks and utilities while preserving inversion-specific functionality
 */
...
import { generateLevel2Progression, validateLevel2Answer, level2Config } from "./level2Utils.js";
...
export default function ChordProgressionsLevel2() {
  // Use shared state and logic (replaces ~300 lines of useState and event handlers)
  const state = useLevelState();
  const logic = useLevelLogic(state, level2Config, {
    generateChord: generateLevel2Progression,
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0a0a0a] via-[#1a1a2e] to-[#16213e] relative">
        ...
        <header className="bg-black/20 backdrop-blur-md border-b border-white/10 px-6 py-4">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center space-x-4 ml-16">
              ...
              <h1 className="text-xl font-bold text-white">{level2Config.title}</h1>
            </div>
            {/* <CompactAuthButton /> */}
          </div>
        </header>
        ...
        <main className="max-w-6xl mx-auto p-6">
          <div className="text-center bg-white/10 backdrop-blur-sm rounded-2xl p-8 mx-auto max-w-2xl">
            <h2 className="text-3xl font-bold text-white mb-6">Ready to Start {level2Config.title}?</h2>
            <div className="text-lg text-white/70 mb-8 space-y-2">
              <p><strong>{level2Config.totalProblems} problems</strong> to complete</p>
              <p>{level2Config.description}</p>
              <p>Need <strong>{level2Config.passAccuracy}% accuracy</strong> to pass</p>
              <p>Average time must be under <strong>{level2Config.passTime} seconds</strong></p>
            </div>
            <button
              onClick={startLevel}
              className={`px-12 py-4 ${level2Config.buttonColor} text-white text-xl font-bold rounded-xl hover:${level2Config.buttonHoverColor} transition-colors shadow-lg`}
            >
              Start {level2Config.title}
            </button>
          </div>
        </main>
      </div>

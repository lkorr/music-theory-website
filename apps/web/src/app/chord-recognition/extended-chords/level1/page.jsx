...
import { generateChord, validateAnswer, levelConfigs } from "../../basic-triads/shared/chordLogic.js";

// Wrapper function to add duplicate prevention
const generateChordWithDuplicatePrevention = (levelConfig, previousChord = null) => {
  let chord, attempt = 0;
  
  // Prevent exact same chord appearing twice in a row
  do {
    chord = generateChord(levelConfig);
    attempt++;
    ...
    if (attempt > 20) break;
           ...
           previousChord.chordType === chord.chordType &&
           previousChord.inversion === chord.inversion);
  
  return chord;
};

// Import shared components
const ChordPianoDisplay = ({ notes, showLabels, setShowLabels }) => {
...
export default function Level5Page() {
  ...
  const TOTAL_PROBLEMS = 40;
  const PASS_ACCURACY = 80; // 80% for 7th chords with inversions
  const PASS_TIME = 10; // 10 seconds for 7th chords with inversions

  const startLevel = () => {
    setHasStarted(true);
    try {
      const chord = generateChordWithDuplicatePrevention(levelConfigs.level5, currentChord);
      console.log('Generated chord:', chord);
      setCurrentChord(chord);
    } catch (error) {
      console.error('Error generating chord:', error);
      console.log('levelConfigs.level5:', levelConfigs.level5);
    }
  ...
  const nextChord = () => {
    try {
      const chord = generateChordWithDuplicatePrevention(levelConfigs.level5, currentChord);
      console.log('Next chord generated:', chord);
      setCurrentChord(chord);
    } catch (error) {
      console.error('Error generating chord in nextChord:', error);
    }
    setUserAnswer('');
    setFeedback(null);
    setIsAnswered(false);
          ...
          <div className="flex flex-col xl:flex-row gap-8 items-start justify-center min-h-[80vh]">
            ...
            <div className="text-center bg-white/20 backdrop-blur-sm rounded-2xl p-8 xl:w-1/3">
              ...
              <div className="text-lg text-black/80 mb-8 space-y-2">
                ...
                <p>Identify 7th chords in all inversions</p>
                <p>Root position, 1st, 2nd, and 3rd inversions</p>
            ...
            {/* Chord Inversion Legend */}
            <div className="bg-white/20 backdrop-blur-sm rounded-2xl p-6 xl:w-2/3 max-h-[80vh] overflow-y-auto">
              <h3 className="text-2xl font-bold text-black mb-6 text-center">7th Chord Inversions</h3>
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  {[
                    { name: 'Root', notation: 'Cmaj7', midiNotes: [60, 64, 67, 71] }, // C4, E4, G4, B4
                    { name: '1st Inv', notation: 'Cmaj7/1', midiNotes: [64, 67, 71, 72] }, // E4, G4, B4, C5
                    { name: '2nd Inv', notation: 'Cmaj7/2', midiNotes: [67, 71, 72, 76] }, // G4, B4, C5, E5
                    { name: '3rd Inv', notation: 'Cmaj7/3', midiNotes: [71, 72, 76, 79] } // B4, C5, E5, G5
                  ].map((chord, i) => {
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  {[
                    { name: 'Root', notation: 'Cm7', midiNotes: [60, 63, 67, 70] }, // C4, Eb4, G4, Bb4
                    { name: '1st Inv', notation: 'Cm7/1', midiNotes: [63, 67, 70, 72] }, // Eb4, G4, Bb4, C5
                    { name: '2nd Inv', notation: 'Cm7/2', midiNotes: [67, 70, 72, 75] }, // G4, Bb4, C5, Eb5
                    { name: '3rd Inv', notation: 'Cm7/3', midiNotes: [70, 72, 75, 79] } // Bb4, C5, Eb5, G5
                  ].map((chord, i) => {
                    // Use dynamic range: 1 note below lowest, 1 note above highest
                    const minNote = Math.min(...chord.midiNotes);
                    const maxNote = Math.max(...chord.midiNotes);
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  {[
                    { name: 'Root', notation: 'C7', midiNotes: [60, 64, 67, 70] }, // C4, E4, G4, Bb4
                    { name: '1st Inv', notation: 'C7/1', midiNotes: [64, 67, 70, 72] }, // E4, G4, Bb4, C5
                    { name: '2nd Inv', notation: 'C7/2', midiNotes: [67, 70, 72, 76] }, // G4, Bb4, C5, E5
                    { name: '3rd Inv', notation: 'C7/3', midiNotes: [70, 72, 76, 79] } // Bb4, C5, E5, G5
                  ].map((chord, i) => {
                    // Use dynamic range: 1 note below lowest, 1 note above highest
                    const minNote = Math.min(...chord.midiNotes);
                    const maxNote = Math.max(...chord.midiNotes);
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  {[
                    { name: 'Root', notation: 'Cdim7', midiNotes: [60, 63, 66, 69] }, // C4, Eb4, Gb4, A4
                    { name: '1st Inv', notation: 'Cdim7/1', midiNotes: [63, 66, 69, 72] }, // Eb4, Gb4, A4, C5
                    { name: '2nd Inv', notation: 'Cdim7/2', midiNotes: [66, 69, 72, 75] }, // Gb4, A4, C5, Eb5
                    { name: '3rd Inv', notation: 'Cdim7/3', midiNotes: [69, 72, 75, 78] } // A4, C5, Eb5, Gb5
                  ].map((chord, i) => {
                    // Use dynamic range: 1 note below lowest, 1 note above highest
                    const minNote = Math.min(...chord.midiNotes);
                    const maxNote = Math.max(...chord.midiNotes);
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                  {[
                    { name: 'Root', notation: 'Cm7♭5', midiNotes: [60, 63, 66, 70] }, // C4, Eb4, Gb4, Bb4
                    { name: '1st Inv', notation: 'Cm7♭5/1', midiNotes: [63, 66, 70, 72] }, // Eb4, Gb4, Bb4, C5
                    { name: '2nd Inv', notation: 'Cm7♭5/2', midiNotes: [66, 70, 72, 75] }, // Gb4, Bb4, C5, Eb5
                    { name: '3rd Inv', notation: 'Cm7♭5/3', midiNotes: [70, 72, 75, 78] } // Bb4, C5, Eb5, Gb5
                  ].map((chord, i) => {
                    // Use dynamic range: 1 note below lowest, 1 note above highest
                    const minNote = Math.min(...chord.midiNotes);
                    const maxNote = Math.max(...chord.midiNotes);
        ...
        <main className="max-w-2xl mx-auto p-6 flex items-center justify-center min-h-[80vh]">
          <div className={`text-center backdrop-blur-sm rounded-2xl p-8 ${
            ...
            <div className="flex gap-4 justify-center">
              ...
              {levelResult.passed && (
                <Link
                  to="/chord-recognition/extended-chords/level2"
      ...
      <main className="max-w-4xl mx-auto p-6">
        ...
        <div className="mt-6 text-center mb-8">
          ...
          <p className="text-black/70 mb-6">
            Type the 7th chord name with inversion (e.g., Cmaj7, Dm7/1, G7/2, Fdim7/3)
          </p>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 mt-12">
          <h3 className="text-lg font-semibold text-black mb-4">Level 5: 7th Chords with Inversions</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h4 className="font-bold text-black mb-2">Chord Types</h4>
              <div className="space-y-1 text-sm text-black/80">
                <div>Major 7th: Cmaj7, M7</div>
                <div>Minor 7th: Cm7, m7</div>
                <div>Dominant 7th: C7</div>
                <div>Diminished 7th: Cdim7, °7</div>
                <div>Half Diminished 7th: Cm7b5, ø7</div>
              </div>
            </div>
            <div>
              <h4 className="font-bold text-black mb-2">Inversions</h4>
              <div className="space-y-1 text-sm text-black/80">
                <div>Root Position: Cmaj7</div>
                <div>1st Inversion: Cmaj7/1</div>
                <div>2nd Inversion: Cmaj7/2</div>
                <div>3rd Inversion: Cmaj7/3</div>
              </div>
            </div>
          </div>
        </div>

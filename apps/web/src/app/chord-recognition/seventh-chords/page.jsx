      ...
      <main className="max-w-4xl mx-auto p-6">
        ...
        <div className="mb-12">
          <div className="text-center mb-6">
            <h3 className="text-2xl font-bold text-white mb-2">7th Chord Recognition</h3>
            <p className="text-white/70">Listen to 7th chords and identify what you hear</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
            {levels.map((level) => (
              <div key={level.id} className="relative">
                <Link
                  to={level.available ? `/chord-recognition/seventh-chords/recognition/${level.id}` : '#'}
        ...
        <div className="mb-12">
          <div className="text-center mb-6">
            <h3 className="text-2xl font-bold text-white mb-2">7th Chord Construction</h3>
            <p className="text-white/70">Build 7th chords by placing notes on the piano roll</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {constructionLevels.map((level) => (
              <div key={level.id} className="relative">
                <Link
                  to={level.available ? `/chord-recognition/seventh-chords/construction/${level.id}` : '#'}
                  className={`block p-6 rounded-2xl transition-all duration-300 transform hover:scale-105 ${
                    level.available 
                      ? 'bg-white/10 backdrop-blur-sm border-2 border-white/20 hover:bg-white/20 cursor-pointer' 
                      : 'bg-white/5 backdrop-blur-sm border-2 border-white/10 cursor-not-allowed opacity-60'
        ...
        <div className="mt-12 text-center">
          <div className="bg-white/10 backdrop-blur-sm border border-white/20 rounded-2xl p-6">
            ...
            <Link
              ...
              className="inline-block px-6 py-3 bg-white/20 text-white font-semibold rounded-xl hover:bg-white/30 transition-colors border border-white/30"
            >
              Review Basic Triads
            </Link>
          </div>

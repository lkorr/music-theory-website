...
interface InversionRules {
  ...
  requireSpecificInversion: string | string[] | number[] | null;
}
 ...
 * @param category - Category (e.g., 'basic-triads')
 * @param level - Level number (e.g., '1' or 1)
 * @returns Level configuration object
 */
 ...
 * @returns Levels grouped by category
 */
export const getLevelsByCategory = (): { [category: string]: (LevelConfig & { levelId: string })[] } => {
  const categories: { [category: string]: (LevelConfig & { levelId: string })[] } = {};
 ...
 */
export const getCategoryInfo = (): CategoryDisplayInfo => {
  return {
    "basic-triads": {
      title: "Basic Triads",

...
import { noteNames, getMidiNoteName, isBlackKey } from "../../../chord-recognition/jazz-chords/shared/chordLogic.js";

// 9th chord types for construction
...
const generateConstructionTask = (previousTask = null) => {
  ...
  const chordType = ninthChordTypes[chordTypeKey];
  
  // Choose octave (C3=48, C4=60)
  const baseOctaves = [48, 60]; // C3, C4
...
// Interactive Piano Roll Component for 9th chords
function InteractivePianoRoll({ placedNotes, onNoteToggle, currentTask, showSolution, feedback, showLabels, setShowLabels }) {
  ...
  const isNoteInChord = (midiNote) => {
    ...
    const expectedSemitones = ninthChordTypes[currentTask.chordType].intervals.map(interval => 
            ...
            <div className="relative h-full">
              ...
              {placedNotes.map((midiNote, index) => {
                ...
                if (feedback && feedback.isCorrect !== undefined) {
                  ...
                  // While placing - show purple for 9th chords
                  noteColorClass = 'bg-purple-500 border-purple-600';
                }
              ...
              {showSolution && currentTask && currentTask.expectedNotes && currentTask.expectedNotes.map((midiNote, index) => {
                ...
                return (
                  <div
                    key={`solution-${index}`}
                    className="absolute bg-purple-400 border-purple-500 rounded-lg shadow-lg opacity-70"
                    style={{
                      left: '20px',
                      top: `${yPos + 2}px`,
                      width: '400px',
      ...
      <div className="grid grid-cols-5 gap-4 text-center">
        ...
        <div>
          <div className={`text-2xl font-bold ${
            avgTime > 0 && avgTime <= 12 ? 'text-green-600' : avgTime > 12 ? 'text-red-600' : 'text-white'
          }`}>
...
export default function Level1NinthChordConstruction() {
  ...
  const PASS_ACCURACY = 80; // 80% for extended chords
  const PASS_TIME = 12; // 12 seconds for more complex chords
  ...
  const validateChord = () => {
    ...
    // 9th chords have 4-5 notes depending on type
    const expectedNoteCount = ninthChordTypes[currentTask.chordType].intervals.length;
    if (placedNotes.length !== expectedNoteCount) return false;
    
    // Convert placed notes to semitone classes (0-11)
    const placedSemitones = placedNotes.map(note => note % 12);
    
    // Get expected chord tones based on the root note
    const rootSemitone = noteNames.indexOf(currentTask.root);
    const expectedSemitones = ninthChordTypes[currentTask.chordType].intervals.map(interval => 
      (rootSemitone + interval) % 12
    );
    ...
    const sortedPlacedSemitones = [...placedSemitones].sort((a, b) => a - b);
    const sortedExpectedSemitones = [...expectedSemitones].sort((a, b) => a - b);
    
    // Check if the placed notes contain exactly the expected chord tones (ignoring octaves)
    return sortedPlacedSemitones.length === sortedExpectedSemitones.length &&
           sortedPlacedSemitones.every((semitone, index) => semitone === sortedExpectedSemitones[index]);
  };
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-start justify-center min-h-[80vh]">
            <div className="text-center bg-white/10 backdrop-blur-sm rounded-2xl p-8 lg:w-1/2">
              ...
              <div className="text-lg text-white/80 mb-8 space-y-2">
                ...
                <p>Build <strong>9th chords</strong> by placing notes on the piano roll</p>
                <p>Need <strong>{PASS_ACCURACY}% accuracy</strong> to pass</p>
                <p>Average time must be under <strong>{PASS_TIME} seconds</strong></p>
              </div>
              <button
                onClick={startLevel}
                className="px-12 py-4 bg-purple-500 text-white text-xl font-bold rounded-xl hover:bg-purple-600 transition-colors shadow-lg"
              >
                Start Building 9th Chords
              </button>
            </div>

            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 lg:w-1/2">
              <h3 className="text-2xl font-bold text-white mb-6 text-center">How to Build 9th Chords</h3>
              <div className="space-y-4 text-white/80">
                <div className="bg-white/10 rounded-xl p-4">
                  <h4 className="font-bold mb-2">Instructions</h4>
                  <p className="text-sm">You'll be asked to build specific 9th chords. Click on the piano roll to place the correct notes.</p>
                </div>
                
                <div className="bg-white/10 rounded-xl p-4">
                  <h4 className="font-bold mb-2">9th Chord Types</h4>
                  <ul className="text-sm space-y-1">
                    <li>• <strong>Major 9th (maj9):</strong> Root + Major 3rd + Perfect 5th + Major 7th + Major 9th</li>
                    <li>• <strong>Minor 9th (m9):</strong> Root + Minor 3rd + Perfect 5th + Minor 7th + Major 9th</li>
                    <li>• <strong>Dominant 9th (9):</strong> Root + Major 3rd + Perfect 5th + Minor 7th + Major 9th</li>
                    <li>• <strong>Dom 7th ♭9 (7♭9):</strong> Root + Major 3rd + Perfect 5th + Minor 7th + Minor 9th</li>
                    <li>• <strong>Add 9 (add9):</strong> Root + Major 3rd + Perfect 5th + Major 9th (no 7th)</li>
                  </ul>
                </div>
                
                <div className="bg-white/10 rounded-xl p-4">
                  <h4 className="font-bold mb-2">Example</h4>
                  <p className="text-sm">For "Cmaj9": Place C, E, G, B, and D notes. Green notes = correct, Red notes = incorrect.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className={`text-center backdrop-blur-sm rounded-2xl p-8 ${
            ...
            <div className="grid grid-cols-2 gap-6 mb-8 text-lg">
              <div className="text-center">
                ...
                <div className="text-gray-700">Accuracy (need {PASS_ACCURACY}%)</div>
              </div>
              <div className="text-center">
                <div className={`text-3xl font-bold ${
                ...
                <div className="text-gray-700">Avg Time (need ≤{PASS_TIME}s)</div>
              </div>
            </div>
            ...
            <div className="flex gap-4 justify-center">
              ...
              {levelResult.passed && (
                <Link
                  to="/chord-construction/jazz-chords/level2"
      ...
      <header className="bg-black/20 backdrop-blur-md border-b border-white/10 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <h1 className="text-xl font-bold text-white">
              Level 1: Build 9th Chords - Problem {score.total}/{TOTAL_PROBLEMS}
            </h1>
          </div>
          ...
          <div className="flex-1">
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-8">
              ...
              <div className="text-center mb-6">
                <div className="text-3xl font-bold text-purple-600 mb-2">{currentTask.chordName}</div>
                <p className="text-white/70">{currentTask.description} in root position</p>
                <p className="text-sm text-white/60 mt-2">
                  Expected notes: {ninthChordTypes[currentTask.chordType].intervals.length}
                </p>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-center gap-4">
                  ...
                  <button
                    ...
                    className="px-8 py-3 bg-purple-500 text-white font-semibold rounded-xl hover:bg-purple-600 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
                  >
                    Check Chord
                  </button>
                </div>
                ...
                {feedback && (
                  <div className={`p-4 rounded-lg ${
                    ...
                    {feedback.isCorrect && (
                      <p className="mt-2">Excellent 9th chord construction!</p>
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="xl:w-80">
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6">
              ...
              <div className="mt-6 pt-6 border-t border-white/20">
                <h4 className="font-semibold text-white mb-2">9th Chord Types:</h4>
                <div className="text-sm text-white/70 space-y-1">
                  <p>• <strong>Major 9th:</strong> Cmaj9, CM9</p>
                  <p>• <strong>Minor 9th:</strong> Dm9, D-9</p>
                  <p>• <strong>Dominant 9th:</strong> C9, G9</p>
                  <p>• <strong>Dom 7♭9:</strong> C7♭9</p>
                  <p>• <strong>Add 9:</strong> Cadd9</p>
                </div>
              </div>

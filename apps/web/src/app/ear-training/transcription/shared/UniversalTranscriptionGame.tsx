...
interface UniversalTranscriptionGameProps {
...
function TranscriptionPianoRoll({ 
  ...
  const handlePianoKeysScroll = (e: React.UIEvent<HTMLDivElement>) => {
    if (pianoRollRef.current) {
      pianoRollRef.current.scrollTop = e.currentTarget.scrollTop;
    }
  };

  const handlePianoRollScroll = (e: React.UIEvent<HTMLDivElement>) => {
    if (pianoKeysRef.current) {
      pianoKeysRef.current.scrollTop = e.currentTarget.scrollTop;
    }
  };
          ...
          <div className="flex">
            ...
            <div 
              ...
              onClick={(e) => {
                ...
                const y = e.clientY - rect.top + (pianoRollRef.current?.scrollTop || 0);
...
function ScoreDisplay({ correct, total, streak, currentTime, avgTime, isAnswered, totalProblems, theme }: ScoreDisplayProps) {
...
export function UniversalTranscriptionGame({ levelConfig }: UniversalTranscriptionGameProps) {
  ...
  const generateTranscriptionTask = (previousTask: TranscriptionTask | null = null): TranscriptionTask => {
    ...
    let root: string, chordTypeKey: string, inversion = 0, baseOctave: number = 60, attempt = 0;
  ...
  const handleNoteToggle = async (midiNote: number) => {
